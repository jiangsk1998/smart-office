...
public class LocalDateTimeConverter implements Converter<LocalDateTime> {
    ...
    private static final DateTimeFormatter FMT1 = DateTimeFormatter.ofPattern("yyyy-M-d HH:mm:ss");
    private static final DateTimeFormatter FMT2 = DateTimeFormatter.ofPattern("yyyy/M/d HH:mm:ss");
    ...
    public LocalDateTime convertToJavaData(ReadCellData<?> cellData, ExcelContentProperty contentProperty, GlobalConfiguration globalConfiguration) {
        ...
        try {
            return LocalDateTime.parse(dateStr, FMT1);
        } catch (DateTimeParseException e) {
            try {
                return LocalDateTime.parse(dateStr, FMT2);
            } catch (DateTimeParseException ex) {
                log.error("无法解析的日期格式: {}. 应为 yyyy-MM-dd HH:mm:ss 或 yyyy/MM/dd HH:mm:ss.", dateStr);
                throw new RuntimeException("无法解析的日期格式: " + dateStr + ". 应为 yyyy-MM-dd HH:mm:ss 或 yyyy/MM/dd HH:mm:ss.", ex);
            }
        }
    }
}
